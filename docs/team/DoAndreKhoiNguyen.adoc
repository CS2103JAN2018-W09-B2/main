...
** Highlights: This enhancement includes eight new command, namely, `calendar`, `addapp`, `delapp`, `date`, `datetime`, `week`, `month` and `year`. It requires integrating with a third-party API, namely, CalendarFX and working with all the main components of contactHeRo. +
